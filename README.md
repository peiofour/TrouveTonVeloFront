...
Now you can rename `src/Header.css` to `src/App.scss` and run `npm run watch-css`. The watcher will find every Sass file in `src` subdirectories, and create a corresponding CSS file next to it, in our case overwriting `src/Header.css`. Since `src/Header.js` still imports `src/Header.css`, the styles become a part of your application. You can now edit `src/App.scss`, and `src/Header.css` will be regenerated.
